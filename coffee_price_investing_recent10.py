...
# 경고 무시 및 멀티스레드 충돌 방지 설정 (실무 배치 환경 권장)
...
# =====================================================================
print("🚀 [Step 1] 데이터베이스 연결 및 최근 10년 커피 가격 데이터 로드 시작...")

con = duckdb.connect('my_database.duckdb')

# 2015년 이후 데이터만 필터링하여 최근 10년의 중장기 모멘텀에 집중합니다.
query = "SELECT 날짜, 종가, 변동 FROM coffee_price_investing WHERE 날짜 > '2015-01-01';"
...
# =====================================================================
print("\n📈 [Step 2] 최근 10년 장기 가격 추세 및 이동평균선 시각화 중...")

# --- 시각화 1: 단순 커피 가격 선그래프 ---
coffee_price_investing.plot(kind='line', y='종가', title='최근 10년 커피 가격 변화 (2015년~)', figsize=(10, 6))
plt.xlabel('Date (Day)')
plt.ylabel('가격')
plt.grid(True)
plt.savefig('최근_10년_커피_가격_기본추세.png', dpi=300, bbox_inches='tight') # 이미지 자동 저장 추가
plt.show()

# --- 파생변수 생성: 50일 & 200일 단순이동평균(SMA) ---
# 이동평균(Moving Average)은 가격의 노이즈를 줄여 추세의 방향을 파악하는 데 쓰입니다.
# 50일 선이 200일 선을 돌파할 때 골든 크로스(강세) / 데드 크로스(약세)를 판단합니다.
...
plt.title('최근 10년 이동평균 (50일 & 200일)')
plt.xlabel('Day')
plt.ylabel('가격')
plt.legend()
plt.grid(True)
plt.savefig('최근_10년_단순이동평균선.png', dpi=300, bbox_inches='tight') # 이미지 자동 저장 추가
plt.show()

# --- 파생변수 생성: 50일 & 200일 지수이동평균(EMA) ---
# 단순이동평균과 달리 '최근 가격'에 더 높은 가중치를 두어 추세 전환 신호를 더 빠르게 반영합니다.
...
plt.title('최근 10년 지수이동평균 (50일 & 200일)')
plt.xlabel('Day')
plt.ylabel('Price')
plt.legend()
plt.grid(True)
plt.savefig('최근_10년_지수이동평균선.png', dpi=300, bbox_inches='tight') # 이미지 자동 저장 추가
plt.show()


# =====================================================================
# [4] 1차 선형 회귀(Linear Regression)를 통한 최근 10년 추세선 분석
# =====================================================================
print("\n📏 [Step 3] np.polyfit 기반 선형 추세선 및 기울기 도출...")

# 결측치(NaN)로 인한 회귀선 오류를 방지하기 위해 dropna() 적용 후 x, y 생성
valid_data = coffee_price_investing['종가'].dropna()
x_trend = np.arange(len(valid_data))
y_trend = valid_data.values

# 1차 방정식(직선)의 계수(기울기, y절편) 탐색
...
# 추세선 매핑 (y_trend 길이에 맞춰 날짜 인덱스를 슬라이싱)
...
plt.title('최근 10년 추세 분석 (선형 회귀)')
plt.xlabel('날짜')
plt.ylabel('가격')
plt.legend()
plt.grid(True)
plt.savefig('최근_10년_커피_가격_추세.png', dpi=300, bbox_inches='tight') # 이미지 자동 저장 추가
plt.show()
...
print(f"\n-> 추세선의 기울기: {slope:.10f}")
if slope > 0:
    print("-> 결론: 데이터는 최근 10년간 '상승'하는 추세를 보입니다.")
elif slope < 0:
    print("-> 결론: 데이터는 최근 10년간 '하락'하는 추세를 보입니다.")
else:
    print("-> 결론: 데이터는 최근 10년간 뚜렷한 추세가 없습니다.")
...
coffee_price_investing['Histogram'] = coffee_price_investing['MACD'] - coffee_price_investing['Signal'] # 히스토그램 (모멘텀)

# --- 2. 볼린저 밴드 (Bollinger Bands) 계산 ---
# 주가의 변동성이 표준정규분포를 따른다고 가정할 때, 가격이 특정 범위 내에서 움직일 확률을 시각화합니다.
coffee_price_investing['MA_20'] = coffee_price_investing['종가'].rolling(window=20).mean() # 중심선 (20일 SMA)
coffee_price_investing['StdDev'] = coffee_price_investing['종가'].rolling(window=20).std()   # 표준편차 (Sigma)
...
ax1.set_title('최근 10년 커피 가격과 볼린저 밴드 (Volatility)', fontsize=16)
ax1.set_ylabel('가격')
...
ax2.set_title('최근 10년 MACD (이동평균 수렴확산)', fontsize=16)
ax2.set_xlabel('날짜')
ax2.set_ylabel('MACD')
...
plt.tight_layout()
plt.savefig('최근_10년_볼린저_밴드_및_MACD.png', dpi=300, bbox_inches='tight') # 이미지 자동 저장 추가
plt.show()

print("\n🎉 최근 10년 기술적 투자 지표 분석 및 시각화가 완료되었습니다!")